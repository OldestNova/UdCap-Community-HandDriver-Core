                ...
                case UDCAP_PROBE_HAND_V1: {
                    ...
                    auto unlisten = core.listen([&core, &ready](const UdCapV1MCUPacket &data) {
                        if (data.commandType == CMD_LINK_STATE) {
                            std::cout << "  Link State: " << UdCapV1Core::fromLinkStateToString(data.linkState) << std::endl;
                            if (data.linkState == LINK_STATE_CONNECTED) {
                            ...
                            std::thread t([&core]() {
                                try {
                                    ...
                                    core.captureCalibrationData(UdCapV1HandCaliType::UDCAP_V1_HAND_CALI_TYPE_ADDUCTION);
                                    std::cout << "Start calibration. Protract!" << std::endl;
                                    std::this_thread::sleep_for(std::chrono::seconds(5));
                                    std::cout << "Getting Value Protract!" << std::endl;
                                    core.captureCalibrationData(UdCapV1HandCaliType::UDCAP_V1_HAND_CALI_TYPE_PROTRACT);
                            ...
                            if (ready)
                                std::cout << "Input joystick. X: " << data.joystickData.joyX << " Y: " << data.joystickData.joyY << std::endl;
                        } else if (data.commandType == CMD_INPUT_BUTTON) {
                            if (ready)
                                std::cout << "Input Button. A: " << data.button.btnA << " B: " << data.button.btnB << " Menu: " << data.button.btnMenu  << " JoyStick: " << data.button.btnJoyStick  << " PWR: " << data.button.btnPower << std::endl;
                        }
                    });
